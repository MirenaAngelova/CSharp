...
class MagicCarNumbers
{
    ...
    static void Main()
    {
        ...
        //Local people are keen to choose special numbers for their cars, know as magic car numbers. They calculate the
        //weight of a car number as follows: they sum its digits and letters, assuming that letters have the following values: 
        //'A'  10, 'B'  20, 'C'  30, 'E'  50, 'H'  80, 'K'  110, 'M'  130, 'P'  160, 'T'  200, 'X'  240. 
        //For example the weight("CA6511BM") = 30 + 10 + 6 + 5 + 1 + 1 + 20 + 130 = 203. A magic car number is a car number 
        //that has a special magic weight (e.g. 256 or 512 for developers) and its number is in some of the formats 
        //"CAaaaaXY", "CAabbbXY", "CAaaabXY", "CAaabbXY", "CAababXY" and "CAabbaXY", where a and b are two different digits 
        ...
        for (int x = 0; x < letters.Length; x++)
        {
            for (int y = 0; y < letters.Length; y++)
            {
                for (int a = 0; a <= 9; a++)
                {
                    CheckCarNumber("CA" + a + a + a + a + letters[x] + letters[y], magicWeight); 
                    for (int b = 0; b <= 9; b++)
                    {
                        if (a != b)
                        {
                            CheckCarNumber("CA" + a + b + b + b + letters[x] + letters[y], magicWeight);
                            CheckCarNumber("CA" + a + a + a + b + letters[x] + letters[y], magicWeight);
                            CheckCarNumber("CA" + a + a + b + b + letters[x] + letters[y], magicWeight);
                            CheckCarNumber("CA" + a + b + a + b + letters[x] + letters[y], magicWeight);
                            CheckCarNumber("CA" + a + b + b + a + letters[x] + letters[y], magicWeight);
                        }
                    }
                }
            }
