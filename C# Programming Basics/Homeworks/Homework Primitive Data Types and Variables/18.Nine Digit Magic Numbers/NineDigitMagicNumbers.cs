...
class NineDigitMagicNumbers
{
    static void Main()
    {
        ...
        //their sum of digits is sum and abc ≤ def ≤ ghi. Numbers holding these properties are also called 
        ...
        //        for (int i = 0; i < 9; i++)
        //        {
        //            if (number[i] - '0' > 7 || number[i] - '0' < 1) //subtract zero from char to get real number
        //            {
        //                resultCount = -1;
        //                break;
        //            }
        ...
        for (int abc = 111; abc <= 777; abc++)
        {
            int def = abc + diff;
            int ghi = def + diff;
            if (isMagicNumber(abc) && isMagicNumber(def) && isMagicNumber(ghi) && ghi <= 777 && sumOfDigits(abc) + 
                sumOfDigits(def) + sumOfDigits(ghi) == sum)
            {
