...
class BitRoller
{
    ...
    static void Main()
    {
        ...
        //on the right (moves its most right bit at the left most position). He tried this several times and he found it is 
        //too easy. Then he invented a more complex game: freeze a certain bit as a pillar and roll right all other bits 
        //several times. Now he is happy but he wants to automate this process.
        //Help Nakov to write a program that rolls r times a 19-bit number n with a frozen bit at position f.
        //Example: we have the number n = 2521, which is 0000000100111011001 in binary (as a 19-bit number). We freeze 
        //the bit at position f = 8 (we count the positions from the right, starting from 0). We roll out the number r = 4 times.
        //We obtain the result 295245 as follows:
    ...
    //    int rolling = int.Parse(Console.ReadLine());
    //    //Console.WriteLine(Convert.ToString(n, 2).PadLeft(BIT, '0'));
    //    for (int i = 0; i < rolling; i++)
    //    {
    //        n = RollBitsRight(n, frozen);
    //        //Console.WriteLine(Convert.ToString(n, 2).PadLeft(BIT, '0'));
    //    }
    //    Console.WriteLine(n);
    //}
    //private static int RollBitsRight(int n, int frozen)
    //{
    //    int rolledN = 0;
    //    for (int i = 0; i < BIT; i++)
    //    {
