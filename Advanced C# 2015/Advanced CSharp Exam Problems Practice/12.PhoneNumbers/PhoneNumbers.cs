...
class PhoneNumbers
{
    ...
    //    //The phone number should be at least two digits long, should start and end with a digit 
    //    ///(optionally, there might be a “+” in front) and might contain the following symbols in it:
    //    /////“(“, “)”, “/”, “.”, “-”, “ “ (left and right bracket, slash, dot, dash and whitespace).
    ...
    static void Main(string[] args)
    {
        ...
        string patternText = 
            "([A-Z]{1,}[A-Za-z]*)[^\\dA-Za-z\\+]*(\\+?[\\d]+[\\d+\\.\\/\\-\\(\\)\\s]*[\\d]+)";
        //string patternText = "([A-Z][A-Za-z]*)[^0-9A-Za-z+]*([+]?[0-9]+[0-9\- \.\/\)\(]*[0-9]+)";
        Regex regexText = new Regex(patternText);
        MatchCollection matches = regexText.Matches(sb.ToString());
        foreach (Match match in matches)
        {
            string name = match.Groups[1].Value;
            string phoneNumber = match.Groups[2].Value;

            string patternPhoneNumber = "[^\\d+\\+]";
            phoneNumber = Regex.Replace(phoneNumber, patternPhoneNumber, String.Empty);
        ...
        foreach (var name in names)
        {
            ...
            counter++;
            Console.Write($"<li><b>{name.Key}:</b> {name.Value}</li>");
            if (names.Count == counter)
            {
                Console.Write("</ol>");
            }
        }
