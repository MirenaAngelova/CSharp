namespace BuhtigIssueTracker2.Interfaces
{
    ...
    /// <summary>
    ///     Defines issue tracker. It can register, login and logout users.
    ///     The tracker contains a set of commands for 
    ///     adding and removing issues. Adds comments to issues
    ///     Gets users own issues and own comments.
    ///     Searches by Tags.
    /// </summary>
    interface IIssueTracker
    {
        ...
        ///     If there is no logged in user, the action returns "There is no currently logged in user"
        /// </returns>
        string CreateIssue(string title, string description, PriorityIssue priority, string[] tags);
