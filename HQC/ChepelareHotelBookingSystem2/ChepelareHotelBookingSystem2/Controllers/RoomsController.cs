namespace ChepelareHotelBookingSystem2.Controllers
{
    ...
    public class RoomsController : Controller
    {
        ...
        /// <returns>Returns IView containing the new room.</returns>
        public IView Add(int venueId, int places, decimal pricePerDay)
            // BUG: Parameter string pricePerDay is passed as string, should be parsed to decimal
        {
            this.Authorize(Roles.VenueAdmin);
            var venue = Data.RepositoryWithVenues.Get(venueId);

            // BUG: The boolean statement was inverted 
            if (venue == null)
            {
        ...
        private void UpdateRoomAvailability(DateTime startDate, DateTime endDate, Room room, 
